...
export default function Waitlist() {
  ...
  function getDemoWaitlist(): WaitlistEntry[] {
    return [
      { id: 'w1', store_id: '0301', guest_name: 'James C.', phone: '555-0101', party_size: 2, status: 'waiting', created_at: new Date(Date.now() - 1000 * 60 * 25).toISOString() },
      { id: 'w2', store_id: '0301', guest_name: 'Elena R.', phone: '555-0202', party_size: 4, status: 'waiting', created_at: new Date(Date.now() - 1000 * 60 * 12).toISOString() },
      { id: 'w3', store_id: '0301', guest_name: 'Marcus V.', phone: '555-0303', party_size: 2, status: 'waiting', created_at: new Date(Date.now() - 1000 * 60 * 5).toISOString() },
    ];
  }
             ...
             >
                <div className="w-12 h-12 bg-[#020617] rounded-lg border border-slate-800 flex items-center justify-center text-[#3ecf8e] shadow-lg">
                  <div className="text-center">
                    <p className="text-[10px] font-bold leading-none">{entry.party_size}</p>
                    <p className="text-[8px] uppercase tracking-tighter">Seats</p>
                  </div>
                </div>

                <div className="flex-1">
                  <h3 className="text-sm font-bold text-white tracking-tight">{entry.guest_name}</h3>
                  <div className="flex items-center gap-4 mt-1 font-mono text-[10px] text-slate-500">
                ...
                <div className="flex items-center gap-3">
                   <button className="px-4 py-2 bg-[#3ecf8e]/10 text-[#3ecf8e] border border-[#3ecf8e]/30 rounded text-[10px] font-bold uppercase tracking-widest hover:bg-[#3ecf8e] hover:text-[#020617] transition-all flex items-center gap-2">
                     <ArrowRight size={12} />
                     Seat Unit
                   </button>
